...
def compute_velocity(
    ...
    The 1e-9 term is only a divide-by-zero guard for near-zero prior centroids.
    """
...
def compute_ns_score(
    ...
    """
    Product heuristic weights (must sum to 1.0):
      velocity*intent 0.25, cross-source 0.20, cohesion 0.15,
      polarization 0.15, centrality 0.15, entropy 0.10.
...
def compute_lifecycle_stage(
    ...
    if current_stage == "Dormant" and velocity_windowed > 0.25:
        return "Emerging"
    if current_stage == "Declining" and velocity_windowed > 0.10:
        return "Growing"
    ...
    if current_stage == "Emerging":
        ...
        if ((days_since_creation >= 5
                and (
                    (entropy is not None and entropy >= 1.2)
                    or (entropy is None and document_count >= 100)
                )
        ...
        logger.warning("Unknown lifecycle stage '%s' — defaulting to Emerging", current_stage)
        return "Emerging"
